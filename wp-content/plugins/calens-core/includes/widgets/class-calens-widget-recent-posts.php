 ...
 * @see WP_Widget
 */
class calens_Widget_Recent_Posts extends WP_Widget {
	...
	public function __construct() {
		$widget_ops = array(
			'classname'                   => 'widget_tcr_recent_entries',
			'description'                 => __( 'Your site&#8217;s most recent Posts.', 'calens-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'calens-recent-posts', __( 'calens Recent Posts', 'calens-core' ), $widget_ops );
		$this->alt_option_name = 'widget_tcr_recent_entries';
	}
	...
	public function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : __( 'Recent Posts', 'calens-core' );
			...
			<?php foreach ( $r->posts as $recent_post ) : ?>
				...
				<li>
					...
					<div class="tcr-post-content">
						...
						<?php if ( $show_date ) : ?>
							<span class="tcr-post-date"><?php echo get_the_date( get_option( 'date_format' ), $recent_post->ID ); ?></span>
						<?php endif; ?>
					</div>
				</li>
			<?php endforeach; ?>
	...
	public function form( $instance ) {
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show:', 'calens-core' ); ?></label>
		...
		<label for="<?php echo esc_attr( $this->get_field_id( 'show_image' ) ); ?>"><?php esc_html_e( 'Display post image?', 'calens-core' ); ?></label></p>
		<?php
	}
}
